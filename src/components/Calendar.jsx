...
const Calendar = ({
                    calendarMonth, setCalendarMonth, calendarYear,
                    setCalendarYear, setVisitingDates, visitingDates,
                  }) => {
  ...
  const calendarDaysNameItems = dayNames.map((item, index) => {
    return (<span className='calendar-days-name' key={index}>{item}</span>);
  });
  ...
  return (
    <div className='calendar'>
      ...
      <div className='calendar-days-box'
           onClick={pickDate}>
        {daysFromPrevMonth}
        {daysInMonth}
      </div>
